...
bool AssetLoader::openArchive(const std::string& path) {
  ...
  for (const auto& [name, entry] : catalog_) {
    ...
    std::cout << "  Checksum: 0x" << std::hex << std::setw(8) 
              << std::setfill('0') << entry.checksum << std::dec << std::endl;
    std::cout << std::endl;
  }
...
std::vector<uint8_t> AssetLoader::loadAsset(const std::string& name) {
  ...
  return decompressData(compressed, entry.originalSize, entry.checksum);
}
...
std::optional<AssetCatalogEntry> AssetLoader::getAssetInfo(const std::string& name) const {
  auto it = catalog_.find(name);
  if (it == catalog_.end()) {
    return std::nullopt;
  }
...
std::vector<uint8_t> AssetLoader::decompressData(
    const std::vector<uint8_t>& data, size_t originalSize, uint32_t expectedChecksum) {
  std::string decompressed;
  if (!snappy::Uncompress(reinterpret_cast<const char*>(data.data()), data.size(),
                     &decompressed)) {
    std::cerr << "Failed to decompress data" << std::endl;
    return {};
  }
  
  std::vector<uint8_t> result(decompressed.begin(), decompressed.end());
  
  // Verify checksum
  uint32_t actualChecksum = calculateCRC32(result);
  if (actualChecksum != expectedChecksum) {
    std::cerr << "Checksum mismatch! Expected: 0x" << std::hex << std::setw(8) 
              << std::setfill('0') << expectedChecksum 
              << " Got: 0x" << std::setw(8) << std::setfill('0') << actualChecksum 
              << std::dec << std::endl;
    return {};
  }
...
uint32_t AssetLoader::calculateCRC32(const std::vector<uint8_t>& data) {
...
void AssetLoader::SaveDirectory(const std::string& directory) {
  ...
  try {
    ...
    for (const auto& [filename, assetType, filepath] : assetRecords) {
      ...
      // Update catalog entry with size, offset, and checksum information
      ...
      catalogEntry.checksum = checksum;
