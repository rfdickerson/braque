...
void Uniforms::CreateUniformBuffers() {
  ...
  for (uint32_t i = 0; i < swapchain.getFramesInFlightCount(); ++i) {
    camera_buffers_.push_back(
        allocator.createBuffer(cameraBufferCreateInfo, cameraBufferAllocInfo));
  }
...
void Uniforms::createDescriptorSets() {
  ...
  for (size_t i = 0; i < camera_buffers_.size(); ++i) {
    ...
    bufferInfo.setBuffer(camera_buffers_[i].buffer);
...
void Uniforms::SetCameraData(vk::CommandBuffer buffer, const Camera& camera) const {
  ...
  const auto& cameraBuffer = camera_buffers_[frame];

  auto& cameraUbo = *static_cast<CameraUbo*>(cameraBuffer.mappedData);
